...
def createTransaction(request):
    if request.method == 'POST':
        fromAdd = request.POST['fromAdd']
        toAdd = request.POST['toAdd']
        amount = request.POST['amount']
        data["funds"][0].createTransaction(transaction.Transaction(fromAdd, toAdd, amount))
        print(data["funds"][0].pendingtransactions)
        return render(request,'transaction.html')
    else:
